...
open class FSPagerView: UIView,UICollectionViewDataSource,UICollectionViewDelegate {
    ...
    open override func layoutSubviews() {
        ...
        if isNormal {
            ...
            self.collectionView.frame = CGRect(x: 0, y: 0, width: self.contentView.bounds.width + 48, height: self.contentView.bounds.height)
            self.contentView.bounds = CGRect(x: 48, y: 0, width: self.bounds.width, height: self.bounds.height)
        }
    }
